...
type Factory interface {
	...
	// Query peer's vault address deployed by this factory.
	GetPeerVault(ctx context.Context, peerID peer.ID) (vault common.Address, err error)
	// Query peer's vault address deployed by this factory. Return cached if cache exists, otherwise query from BTTC.
	GetPeerVaultWithCache(ctx context.Context, peerID peer.ID) (vault common.Address, err error)
	// Check whether the factory is compatible with the `peerID's` factory.
...
/*
@Title IsPeerFactoryCompatible
@description Check whether the factory is compatible with the `peerID's` factory. Note that
	the peers cannot upload file and pay cheque to each other if their VaultFactory
	version doesn't same. Because they cann't pay and cash cheque under current version.
*/
